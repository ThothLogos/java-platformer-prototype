package com.highasacat.dinnerwithdanger.graphics;

import java.util.Random;

import com.highasacat.dinnerwithdanger.level.tiles.Tile;
import com.highasacat.dinnerwithdanger.graphics.Sprite;



public class Screen {
	
	public int height;
	public static int width;
	public int[] pixels;
	public int xOffset, yOffset;
	public final int MAP_SIZE = 4;
	public final int MAP_SIZE_MASK = MAP_SIZE -1;
	public int[] tiles = new int[MAP_SIZE * MAP_SIZE];
	public final int TILE_RES_2N = 5; // 2^n resolution
	public final int TILE_RES = 32;
	public final int TILE_MASK = TILE_RES - 1;
	
	private Random random = new Random();
	
	
	public Screen(int width, int height) {
		this.width = width;
 		this.height = height;
		
		pixels = new int[width * height];
		
		//for (int i = 0; i < (MAP_SIZE * MAP_SIZE); i++) {
		//	tiles[i] = random.nextInt(0xFFFFFF);
		//	tiles[0] = 0;
		//}
	}
	


	public void renderTile(int xp, int yp, Tile tile) {
		xp -= xOffset;
		yp -= yOffset;
		for (int y = 0; y < tile.sprite.SIZE; y++) {
			int ya = y + yp;
			for (int x = 0; x < tile.sprite.SIZE; x++) {
				int xa = x + xp;
				if (xa < -tile.sprite.SIZE || xa >= width || ya < 0 || ya >= height) break;
				if (xa < 0) xa = 0;
				pixels[xa+ya*width] = tile.sprite.pixels[x+y*tile.sprite.SIZE];
			}
		}
	}
	
	public void renderPlayer(int xp, int yp, Sprite sprite) {
		xp -= xOffset;
		yp -= yOffset;
		for (int y = 0; y < TILE_RES; y++) {
			int ya = y + yp;
			for (int x = 0; x < TILE_RES; x++) {
				int xa = x + xp;
				if (xa < -TILE_RES || xa >= width || ya < 0 || ya >= height) break;
				if (xa < 0) xa = 0;
				int col = sprite.pixels[x + y * TILE_RES];
				if (col != 0xffff4fb5) pixels[xa+ya*width] = col;
			}
		}
	}
	
	public void clear() {
		for (int j = 0; j < height; j++) {
			pixels[j] = 0;
		}
	}
	
	public void setOffset(int xOffset, int yOffset) {
		this.xOffset = xOffset;
		this.yOffset = yOffset;
	}
}
